...
reassign_indices(CState) ->
    reassign_indices(CState, [], opt_utils:get_current_time(), fun no_log/2).
...
init([]) ->
    schedule_tick(),
    {ok, #state{changes=[], seed=opt_utils:get_current_time()}}.
    ...
    case maybe_commit_staged(State) of
        {ok, _} ->
            State2 = State#state{next_ring=undefined,
                                 changes=[],
                                 seed=opt_utils:get_current_time()},
            {ok, State2};
        not_changed ->
            {error, State};
        {not_changed, Reason} ->
...
clear_staged(State) ->
    ...
    State#state{changes=[], seed=opt_utils:get_current_time()}.
...
do_maybe_force_ring_update(Ring) ->
    case compute_next_ring([], opt_utils:get_current_time(), Ring) of
        {ok, NextRing} ->
...
internal_ring_changed(Node, CState) ->
    ...
    case {IsClaimant, riak_core_ring:cluster_name(CState5)} of
        {true, undefined} ->
            ClusterName = {Node, opt_utils:get_current_time()},
...
do_claimant(Node, CState, Log) ->
    do_claimant(Node, CState, [], opt_utils:get_current_time(), Log).

...
new_segment_store(Opts, State) ->
    DataDir = case proplists:get_value(segment_path, Opts) of
                  undefined ->
                      Root = "/tmp/anti/level",
                      <<P:128/integer>> = md5(term_to_binary({opt_utils:get_current_time(), make_ref()})),
    ...
    {Offset, _} = random:uniform_s(1 + WriteBufferMax - WriteBufferMin, opt_utils:get_current_time()),
